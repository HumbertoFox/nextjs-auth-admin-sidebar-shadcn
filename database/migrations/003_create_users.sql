...
CREATE TABLE IF NOT EXISTS users (
    ...
    session_version      INT NOT NULL DEFAULT 1,              -- Versão da sessão para invalidação de tokens
    ...
    role                 user_role NOT NULL DEFAULT 'CLIENT', -- Nível de acesso do usuário
    email_verified       TIMESTAMPTZ NULL,                    -- Data/hora de verificação do email
    must_change_password BOOLEAN NOT NULL DEFAULT FALSE,      -- Indica se o usuário deve alterar a senha no próximo login
...
COMMENT ON COLUMN users.session_version      IS 'Versão da sessão para invalidação de tokens';
...
COMMENT ON COLUMN users.role                 IS 'Nível de acesso: ADMIN, USER ou CLIENT';
COMMENT ON COLUMN users.email_verified       IS 'Timestamp de verificação do email (NULL = não verificado)';
COMMENT ON COLUMN users.must_change_password IS 'Indica se o usuário deve alterar a senha na próxima entrada';
